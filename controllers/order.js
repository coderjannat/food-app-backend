import { asyncError } from "../middleswares/errorMiddleware.js";
...
export const placeOrder = asyncError(async (req, res, next) => {
  ...
  const user = "req.user._id"; //for testing in Postman
...
export const getMyOrders = asyncError(async (req, res, next) => {
  const orders = await Order.find({
    user: req.user._id,
  }).populate("user", "name"); //populate means that the "user" place order, give that user "name"
...
// user get specific order detail
...
// change order status (shipped, delivered)
